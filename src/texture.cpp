...
std::unique_ptr<Texture2d> Texture2d::Create(int width, int height, uint32_t inner_format,
...
void Texture2d::SetTextureFromImage(const Image* image) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, inner_format_, width_, height_, 0, format_, type_,
                 image->data());
    glGenerateMipmap(GL_TEXTURE_2D);
}

void Texture2d::SetTextureFormat(int width, int height, uint32_t inner_format, uint32_t format,
                                 uint32_t type) {
    width_ = width;
    height_ = height;
...
std::unique_ptr<Texture3d> Texture3d::Create(const std::vector<Image*>& images) {
...
std::unique_ptr<Texture3d> Texture3d::Create(int width, int height, int length, uint32_t format,
                                             uint32_t type) {
    auto texture = std::unique_ptr<Texture3d>(new Texture3d());
    texture->Bind();
    texture->SetFilter(GL_LINEAR, GL_LINEAR);
...
void Texture3d::SetCubeMapFromImages(const std::vector<Image*>& images) {
    ...
    for (uint32_t i = 0; i < (uint32_t)images.size(); i++) {
        ...
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, image->width(), image->height(),
                     0, format, type_, image->data());
    }
}

void Texture3d::SetCubeMapFormat(int width, int height, int length, uint32_t format,
                                 uint32_t type) {
    width_ = width;
    height_ = height;
    length_ = length;
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, format_, height_, length_, 0, format_, type_,
                 NULL);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, format_, height_, length_, 0, format_, type_,
                 NULL);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, format_, width_, height_, 0, format_, type_,
                 NULL);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, format_, width_, height_, 0, format_, type_,
                 NULL);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, format_, length_, width_, 0, format_, type_,
                 NULL);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, format_, length_, width_, 0, format_, type_,
                 NULL);
}