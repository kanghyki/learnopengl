...
class BaseTexture {
  public:
    BaseTexture(uint32_t texture_type) : texture_type_(texture_type) { glGenTextures(1, &id_); }
    virtual ~BaseTexture() {
        if (id_) {
    ...
    virtual void SetWrap(uint32_t s_wrap, uint32_t t_wrap, uint32_t r_wrap = GL_NONE) const final {
...
class Texture2d : public BaseTexture {
  public:
    ...
    static std::unique_ptr<Texture2d> Create(int width, int height, uint32_t inner_format = GL_RGBA,
    ...
    void SetTextureFormat(int width, int height, uint32_t inner_format, uint32_t format,
                          uint32_t type);
    ...
    inline uint32_t inner_format() const { return inner_format_; }
...
class Texture3d : public BaseTexture {
  public:
    ...
    static std::unique_ptr<Texture3d> Create(int width, int height, int length, uint32_t format,
                                             uint32_t type);
    ~Texture3d();
  ...
  private:
    ...
    void SetCubeMapFormat(int width, int height, int length, uint32_t format, uint32_t type);
